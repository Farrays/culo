 ...
 * con fecha a 48h para probar el sistema de recordatorios
 *
 * GET /api/test-booking-reminder?phone=34622247085
 *
 * Query params:
...
export default async function handler(
  ...
  try {
    ...
    // Crear fecha exactamente 48h desde ahora
    const classDate = new Date(Date.now() + 48 * 60 * 60 * 1000);
    ...
    return res.status(200).json({
      ...
      instructions: {
        testCron: 'GET /api/cron-reminders',
        deleteBooking: `/api/test-booking-reminder?action=delete`,
      },
    });
  } catch (error) {
