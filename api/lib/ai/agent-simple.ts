...
const VALID_STATIC_URL_PATTERNS: RegExp[] = [
  ...
  /^https?:\/\/momence\.com\/sign-in/i,
  // Images/assets
  /^(https?:\/\/)?(www\.)?farrayscenter\.com\/images\//i,
];

/**
 * Extracts all URLs from a JSON string (tool result).
 * Used to collect URLs generated by tools so the sanitizer can allow them.
 */
 ...
 * Only allows: (1) URLs returned by tools, (2) known static website pages.
 * Momence URLs are ONLY valid if they came from a tool result.
 */
function sanitizeUrls(
  ...
  sanitized = sanitized.replace(urlRegex, matchedUrl => {
    ...
    // Check 1: Is this URL from a tool result? (exact match or starts-with for truncation)
    for (const toolUrl of toolGeneratedUrls) {
      if (
        normalized === toolUrl ||
        toolUrl.startsWith(normalized) ||
        normalized.startsWith(toolUrl)
      ) {
        return matchedUrl; // Tool-generated URL — safe
      }
    }

    // Check 2: Is this a known static website page?
    const isStaticValid = VALID_STATIC_URL_PATTERNS.some(pattern => pattern.test(normalized));
    if (isStaticValid) {
      return matchedUrl;
    }

    // FABRICATED URL — replace with general page
...
async function handleResponse(params: {
  ...
  const afterUrlSanitization = sanitizeUrls(assistantMessage, language, toolGeneratedUrls);
...
export async function processSimpleMessage(
  ...
  try {
    ...
    while (currentResponse.stop_reason === 'tool_use' && iterations < MAX_TOOL_ITERATIONS) {
      ...
      // Execute tools and collect results + URLs
      ...
      for (const toolUse of toolUseBlocks) {
        ...
        // Collect URLs from tool results so the sanitizer can allow them
        for (const url of extractUrlsFromToolResult(result)) {
          toolGeneratedUrls.add(url);
        }
        toolResults.push({
          type: 'tool_result',
