...
function extractTranslations(content, locale) {
  const translations = {};

  // Encontrar el inicio del objeto (ajustar para cada idioma)
  const match = content.match(new RegExp(`export\\s+const\\s+${locale}\\s+=\\s+\\{`));
  if (!match) {
    throw new Error(`No se encontró "export const ${locale} = {" en el archivo`);
  }
...
// Función para clasificar una key (IGUAL que en split-translations.mjs)
function classifyKey(key) {
  ...
  if (key.startsWith('hero_') ||
      ...
      key.startsWith('testimonial') && !key.startsWith('testimonial_') ||
...
for (const locale of LOCALES) {
  ...
  // Leer el archivo
  const filePath = path.join(__dirname, `../i18n/locales/${locale}.ts`);
  ...
  console.log(`Extrayendo traducciones del archivo ${locale}.ts...`);
...
console.log(`\n${'='.repeat(60)}`);
console.log('✅ TODOS LOS IDIOMAS PROCESADOS');
console.log('='.repeat(60));
