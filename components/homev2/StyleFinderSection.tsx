      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {config.personas.map((persona, index) => {
            ...
            return (
              <AnimateOnScroll key={persona.id} delay={100 + index * 100}>
                <Link to={`/${locale}${persona.href}`} className="group block h-full">
                  <div
                    className={`h-full bg-gradient-to-br ${persona.gradient} p-[1px] rounded-2xl hover:scale-[1.02] transition-all duration-300`}
                  >
